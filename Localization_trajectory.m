function [q_time, theta_t, time, P_current, covar_det, P_cell, counter] = Localization_trajectory(q_start, q_end, P_init, P_cell, covar_det, odometric_noise, map, limits, init_q, old_q, init_theta, old_theta, time, xLM, yLM, instr_var_noise, instr_noise, maxlen, kalman, counter)
...
%         P_current
        ...
        if kalman == 1
            [q_current, P_current] = EKF_([old_q(num,:) old_theta(num,:)]', q_current_est, P_current_est, xLM, yLM, instr_var_noise, instr_noise, maxlen);
        else
        ...
        if d == 10000000000
            ...
            plot_trajectory(map, q_time(1:counter, :), limits, 'y')
            d = 0;
